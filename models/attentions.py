...
Modified from LayerNorm to RMSNorm for all attention modules"""
...
from flax.nnx.nn.normalization import LayerNorm, RMSNorm
...
class MultiHeadAttention(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.query_rn: RMSNorm | None
        self.key_rn: RMSNorm | None
        if self.normalize_qk:
            # Normalizing query and key projections stabilizes training with higher
            # LR. See ViT-22B paper http://arxiv.org/abs/2302.05442 for analysis.
            self.query_rn = RMSNorm(
                self.head_dim,
                use_bias=False,
                dtype=self.dtype,
                param_dtype=self.param_dtype,
            ...
            self.query_rn = nnx.data(None)
            self.key_rn = nnx.data(None)
    ...
    def __call__(
        ...
        *,
        ...
        if self.normalize_qk:
            assert self.query_rn is not None and self.key_rn is not None
            # Normalizing query and key projections stabilizes training with higher
            # LR. See ViT-22B paper http://arxiv.org/abs/2302.05442 for analysis.
            query = self.query_rn(query)
            key = self.key_rn(key)
...
def apply_rotary_emb(query, key, freqs_cis, seq_len, offset: int = 0):
    ...
    q_complex = q_[..., 0] + 1j * q_[..., 1]
    k_complex = k_[..., 0] + 1j * k_[..., 1]
    ...
    k_out = to_real(k_rot).reshape(*key.shape).astype(key.dtype)
        ...
        *,
        ...
        if self.dropout_rate > 0.0:  # Require `deterministic` only if using dropout.
            deterministic = first_from(
                deterministic,
                self.deterministic,
                error_msg="""No `deterministic` argument was provided to MultiHeadAttentionRoPE
          as either a __call__ argument, class attribute, or nnx.flag.""",
            )
            if not deterministic:
                if rngs is None:
                    raise ValueError(
                        "'rngs' must be provided to __call__ method if "
                        "MultiHeadAttentionRoPE instance is defined with keep_rngs=False."
                    )
                dropout_rng = rngs()
            else:
                dropout_rng = None
...
def combine_masks(*masks: Array | None, dtype: Dtype = jnp.float32) -> Array | None:
